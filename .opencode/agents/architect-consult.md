...
- To create a new ADR file: use `bash` with a heredoc, e.g. `cat > docs/architecture/adr/ADR-NNN-<slug>.md <<'EOF' ... EOF`.
- To modify an existing ArchSpec section: use `ast_grep_replace` for structural edits, or `bash` with `sed -i` / `perl -i` for line-level edits. For markdown, `sed -i` and `perl -i` are usually enough.
