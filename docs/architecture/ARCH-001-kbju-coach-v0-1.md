...
- Image sources of truth: the `openclaw-gateway` service image is the canonical OpenClaw container at `ghcr.io/openclaw/openclaw` (upstream <https://github.com/openclaw/openclaw>; same source as `docs/knowledge/openclaw.md` and §0.1 above). All operator-installed gateway images MUST resolve from this canonical path. Any alternative path (forks, private rebuilds) is out of scope for ARCH-001@0.7.2 and would require a new ADR. The image is pinned per `image@sha256:<digest>` form per TKT-043@0.1.0; the digest is publicly retrievable via `docker manifest inspect ghcr.io/openclaw/openclaw:latest` at PR-merge time.
...
- 2026-05-25 (v0.7.2): Architect-consult patch (in-flight, Q-TKT-043-01). §10.2 Runtime Topology amended with explicit `Image sources of truth` bullet pinning the canonical OpenClaw gateway image to `ghcr.io/openclaw/openclaw` per `docs/knowledge/openclaw.md` and §0.1. Resolves drift between docker-compose.yml's pre-patch `ghcr.io/nicholasgriffintn/openclaw` reference (which does not resolve on GHCR) and every authoritative source. Patch only — no Component / ADR retirement, no design change. Branch arch/ARCH-001-openclaw-gateway-canonical-image. status: approved → draft (PO sets status: approved on merge).
